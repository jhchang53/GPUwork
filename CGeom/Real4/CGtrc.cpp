...
void CGtrc::TRC(const float R0_i, const float R1_i,
  const float Z0_i, const float Z1_i)
{
  R0 = R0_i;   R1 = R1_i;
  Z0 = Z0_i;   Z1 = Z1_i;
...
int CGtrc::track(const float p[], const float w[], float trk[])
{
  trk[0] = INF;  trk[1] = -INF;
  ...
  float Mw0 = -gamsq*wx; float Mw1 = -gamsq*wy; float Mw2 = (1-gamsq)*wz;
  float Md0 = -gamsq*px; float Md1 = -gamsq*py; float Md2 = (1-gamsq)*(pz-Vz);
  float c0 = px*Md0+py*Md1+(pz-Vz)*Md2;
  float c1 = wx*Md0+wy*Md1+wz*Md2;
  float c2 = wx*Mw0+wy*Mw1+wz*Mw2;
  if(prlev) printf("c0=%.3le c1=%.3le c2=%.3le",c0,c1,c2);
  float det = c1*c1-c0*c2;
