...
void CGrpp::RPP(const double a[], const double b[])
{
  for(int i=0; i < 3; i++) {
    box_C[i] = (b[i]+a[i])/2;
    box_e[i] = (b[i]-a[i])/2;
...
int CGrpp::Clip(double denom, double numer, double &t0, double &t1)
{
...
int CGrpp::track(const double p[], const double w[], double trk[])
{
