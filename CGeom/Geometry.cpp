...
Ray Geometry::track(const double p[], const double w[])
{
  Ray rc;
  rc.dist = INF;
  /*  p, w is UV  */
...
Ray Geometry::CGtrack(double p[], double w[])
{
  Ray ray;
  ...
  for(int n=0; n < ncgs; n++) {
    ...
    if(prlev >1) printf("CG%d: ok=%d trk=%.3le %.3le\n",n,ok,trk[0],trk[1]);
    isok[n] = ok;
    if(ok) {
      t_min[n] = trk[0];
      t_max[n] = trk[1];
...
/*  UV geometry  */
void Geometry::setLattice(int nxyz[], double dxyz[], char *uv)
{
...
Ray Geometry::UVtrack(double p[], double w[], char chx)
{
